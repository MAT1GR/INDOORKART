...
router.get('/dashboard', async (req, res) => {
  try {
    ...
    const occupancyRate = totalCapacity > 0 ? 
      Math.round(((occupiedSeats._sum.qty || 0) / totalCapacity) * 100) : 0;
...
router.post('/plans', requireRole(['admin']), async (req, res) => {
  try {
    const { id, name, qualyLaps, raceLaps, description, active = true } = req.body;
...
router.post('/plans/:planId/prices', requireRole(['admin']), async (req, res) => {
  try {
    ...
    const revenueByPeriod = bookings.reduce((acc, booking) => {
      ...
      booking.payments.forEach(payment => {
        if (payment.status === 'completed') {
          acc[key][payment.method as keyof typeof acc[string]] += payment.amount;
        }
      });
